...
export function SalesSummaryCards({ dateFrom, dateTo }: { dateFrom: string; dateTo: string }) {
  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        {[...Array(5)].map((_, i) => (
          <div key={i} className="p-6 rounded-xl border border-[#262626] bg-[#0a0a0a] animate-pulse h-[116px]"></div>
        ))}
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
      {cards.map((c, i) => (
        <div key={i} className="p-5 rounded-xl border border-[#262626] bg-[#0a0a0a] hover:border-[#c5a059]/30 hover:bg-[#141414] transition-all shadow-sm flex flex-col justify-between group">
          <div className="flex justify-between items-start mb-2">
            <h4 className="text-sm font-medium text-gray-400 group-hover:text-gray-300 transition-colors">{c.title}</h4>
            <div className="p-2 bg-[#141414] border border-[#262626] rounded-lg group-hover:border-[#c5a059]/20 transition-colors">{c.icon}</div>
          </div>
