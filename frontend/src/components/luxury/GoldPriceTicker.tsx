...
import { publicApi, type PublicGoldPrice } from "@/lib/publicApi"
...
export function GoldPriceTicker() {
  const { data: prices, isLoading } = usePublicGoldPrices()

  const getPrice = (karat: number) => {
    const p = prices?.find((p) => p.karat === karat)
    return p ? Number(p.price_per_gram).toLocaleString("ar-EG") : "---"
  }
