...
def test_inventory_state_machine_and_ledger(client: TestClient, admin_token_headers):
    ...
    # 2. Sell item is not explicitly built in API for inventory, but let's test ADJUST
    ...
    assert len(txs) == 2
    assert txs[1]["transaction_type"] == TransactionType.ADJUST.value
    assert txs[1]["historical_weight"] == "9.900" 

    # 3. Admin can list all transactions
    all_txs_resp = client.get("/api/v1/inventory-transactions", headers=admin_token_headers)
    assert all_txs_resp.status_code == 200
    assert len(all_txs_resp.json()) >= 2
